    ...
    <table class="cart-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>RWF <?= number_format((float)$p['price'], 0, '.', ',') ?> / <?= $p['unit_type'] ?></td>
                <td>
                    <?= $p['unit_type'] === 'kg'
                        ? number_format((float)$ci['quantity'], 1, '.', ',') . ' kg'
                        : (int)$ci['quantity'] . ' unit(s)' ?>
                </td>
                <td><strong>RWF <?= number_format((float)$ci['subtotal'], 0, '.', ',') ?></strong></td>
                <td>
        ...
        <tfoot>
            <tr>
                ...
                <td><strong style="color:var(--green-mid);font-size:1.1rem;">RWF <?= number_format((float)$total, 0, '.', ',') ?></strong></td>
                <td></td>
            </tr>
        </tfoot>
    </table>
        ...
        <form method="post" action="checkout.php">
            ...
            <div style="display:flex;gap:12px;align-items:center;">
                <button type="submit" class="btn-primary" style="padding:12px 32px;font-size:1rem;">
                    ✅ Place Order — RWF <?= number_format((float)$total, 0, '.', ',') ?>
                </button>
